...
EOF
    ...
    echo LINUX_KERNEL_CMDLINE="\"console=ttyS2,1500000 console=tty1 rootwait rw cgroup_enable=cpuset cgroup_memory=1 cgroup_enable=memory ${bootargs}"\" > ${mount_point}/writable/etc/defaults/flash-kernel
    echo LINUX_KERNEL_CMDLINE_DEFAULTS="\""\" >> ${mount_point}/writable/etc/defaults/flash-kernel
